...
public class NoteAddUpdateActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        noteHelper =
                NoteHelper.getInstance(getApplicationContext());
        noteHelper.open();
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.btn_submit) {
            ...
            if (isEdit) {
                long result =
                        noteHelper.update(String.valueOf(note.getId()), values);
                if (result > 0) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (isEdit) {
            getMenuInflater().inflate(R.menu.menu_form,
                    menu);
        }
    ...
    private void showAlertDialog(int type) {
        final boolean isDialogClose = type ==
                ALERT_DIALOG_CLOSE;
        ...
        alertDialogBuilder
                ...
                .setPositiveButton("Ya", (dialog, id) -> {
                    if (isDialogClose) {
                        ...
                        long result =
                                noteHelper.deleteById(String.valueOf(note.getId()));
                        if (result > 0) {
                            Intent intent = new Intent();
                            intent.putExtra(EXTRA_POSITION, position);
